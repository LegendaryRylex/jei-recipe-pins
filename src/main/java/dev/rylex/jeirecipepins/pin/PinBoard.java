...
public final class PinBoard {
    ...
    private void load(Minecraft minecraft) {
        ...
        for (PinStore.Entry entry : store.load()) {
            runtime.getRecipeManager()
                    .getRecipeType(ResourceLocation.parse(entry.type()))
    ...
    private <T> Optional<PinnedRecipe> decode(RecipeType<T> type, JsonElement recipe, RegistryOps<JsonElement> ops) {
